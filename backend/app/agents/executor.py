...
def execute(incident_id: str, service: str, recommendation: Recommendation) -> IncidentReport:
    if DIRECT_MODE:
        return _execute_direct(incident_id, service, recommendation)
    return _execute_via_llm(incident_id, service, recommendation)


def _execute_direct(incident_id: str, service: str, recommendation: Recommendation) -> IncidentReport:
    start = time.time()
    ...
    return IncidentReport(
        ...
        action_taken=recommendation.options[0].action if recommendation.options else "Rolled back service",
...
def _execute_via_llm(incident_id: str, service: str, recommendation: Recommendation) -> IncidentReport:
    ...
    agent = LlmAgent(
        ...
        instruction="""Execute the approved remediation using the rollback tool, then
re-check the service via Grafana tools to confirm recovery, then produce a final
incident report with concrete recovered-metric values.""",
